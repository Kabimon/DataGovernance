...
CREATE TABLE `g8e_dw_layer`  (
    ...
    name_alias varchar(64) not null unique comment '英文名',
    description varchar(255),
    authority text,
    preset bit not null default 0 comment '0 自定义 1 预置',
    enabled bit not null default 1,
    auto_collect_strategy text comment '自动收纳策略：JSON 字符串',
    available_dbs varchar(64) comment '可用库，ALL: 代表所有，逗号隔开',
    charge_user varchar(64) not null comment '负责人',
...
CREATE TABLE `g8e_dw_subject_domain`  (
    ...
    name_alias varchar(64) not null unique comment '英文名',
    description varchar(255),
    authority text comment '预留字段',
    available_roles text comment '可用角色，逗号隔开',
    enabled bit not null default 1,
    charge_user varchar(64) not null comment '负责人',
    create_user varchar(32) not null,
    create_time datetime not null,
...
CREATE TABLE `g8e_dw_stat_cycle`  (
    ...
    available_roles text comment '可用角色 ALL 所有',
